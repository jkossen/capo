...
class GraphSelectionItemRepository extends BaseEntityRepository
{
    ...
    public function repositionItem(Array $data)
    {
        ...
        if ($this->_fixItemNumbering($data)) {
            // since doctrine caches data, we need to retrieve the possibly new
            // itemnr the hard way
            $q->select('e.itemnr');
            $q->where('e.id = ' . $item->getId());
            $query = $q->getQuery();
            $res = $query->getSingleResult();
            $item->setItemNr($res['itemnr']);
        }
